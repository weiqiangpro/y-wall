...
$(document).scroll(function () {
    ...
    if(CheckIsLoad()){
        if(isempty == true){
            ...
            $.ajax({
                url:"https://www.aoteam.top/api/article/follow.do/"+pagenum,
                type:"GET",
                headers:{
                    Token:token
                },
...
function FirstGetImg() {
    ...
    $.ajax({
        url:"https://www.aoteam.top/api/article/follow.do/1",
        type:"GET",
        headers:{
            Token:token
        },
...
function AddImg(data) {
    ...
    for(var i = 0;i<data.length;i++){
        ...
        var des = $("<div class='description'></div>");
        des.append(data[i].title);

        var author = $("<div class='author'></div>");
        var author_head = $("<img class='head-photo-min'>");
        author_head.attr("src",data[i].pho);
        var author_name = $("<div style='height: 40px;margin-top: 5px;'></div>");
        var name = $("<a class='username'></a>");
        name.append(data[i].userName);
        name.attr("href","homepage.html?id="+data[i].userId);
        author_name.append(name);
        author.append(author_head);
        author.append(author_name);

        time.appendTo(parentdiv);
        button.attr("imgid",imgid);
        button.appendTo(parentdiv);
        des.appendTo(parentdiv);
